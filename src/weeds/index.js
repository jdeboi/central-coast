...
const bgColor = 0x008844;
const sunColor = 0xeeeeee;
...
function init() {
  ...
  const skyboxTexture = cubeLoader.load([
    "./skybox/milkyway/dark-s_px.jpg", // Positive X (right)
    "./skybox/milkyway/dark-s_nx.jpg", // Negative X (left)
    "./skybox/milkyway/dark-s_py.jpg", // Positive Y (top)
    "./skybox/milkyway/dark-s_ny.jpg", // Negative Y (bottom)
    "./skybox/milkyway/dark-s_pz.jpg", // Positive Z (front)
    "./skybox/milkyway/dark-s_nz.jpg", // Negative Z (back)
  ]);

  // Set the scene's background to the loaded skybox texture
  //   scene.background = skyboxTexture;

  // Load the texture using TextureLoader
  const textureLoader = new THREE.TextureLoader();
  const texture = textureLoader.load("./water.png"); // Replace with your texture path

  // Create a material with the texture, enabling transparency
  const material = new THREE.MeshBasicMaterial({
    map: texture, // Apply the texture
    transparent: true, // Enable transparency
    opacity: 0.5, // Set the transparency level (0 is fully transparent, 1 is fully opaque)
  });
  ...
  plane.position.set(0, 50, 100); // Adjust position if needed
  //   scene.add(plane);

  // tree
  //   const loader = new OBJLoader();
  const loader = new GLTFLoader();
  //   loader.load("animated_kelp.glb", function (object) {
  //     object.position.set(0, -150, -150);
  //     object.scale.multiplyScalar(400);
  //     scene.add(object);
  //   });
  loader.load(
    "./animated_kelp.glb", // Replace with the actual path to your .glb file
    function (gltf) {
      const object = gltf.scene;
      object.position.set(0, -1, 0); // Adjust position as necessary
      object.scale.set(100, 100, 100); // Adjust scale as necessary
      scene.add(object);
    },
  ...
  water.material.uniforms["color"].value.set(0x0000ff);
  water.material.uniforms["config"].value.w = 100;
  water.material.uniforms["flowDirection"].value.x = 0.5;
  water.material.uniforms["flowDirection"].value.normalize();
  water.material.uniforms["flowDirection"].value.y = 0.5;
  water.material.uniforms["flowDirection"].value.normalize();
